...
class VodafoneService {
  ...
  /// 1. get_seamless - نفس دالة get_seamless() في البايثون:42
  Future<({String seamless, String msisdn})> getSeamless() async {
    ...
    final resp = await http.get(url, headers: headers).timeout(const Duration(seconds: 30));
  ...
  Future<String> getToken(String seamless) async {
    ...
    final resp = await http.post(url, headers: headers, body: body).timeout(const Duration(seconds: 30));
  ...
  Future<Map<String, dynamic>> sendOrder({
    ...
    final resp = await http.post(url, headers: headers, body: jsonEncode(payload)).timeout(const Duration(seconds: 30));
